...
// names, codes) take an /* i18n-exempt */ on the same line.
{
  ...
  // 30-60s agent repair loop per build (add de key, add en key, replace the
  // literal — three edits per string, plus failed-anchor retries), while the
  // backend already owns an AST-grade migrator (scripts/i18n-migrate.mjs +
  // one translation call) that heals leftovers mechanically after the agent
  // finishes — including template literals these regexes never saw. The
  // agent still writes makeT-first (skeleton, SKILL, prompt all teach it);
  // whatever slips through is pipeline input, not the agent's problem.
  if (literalHits.length) {
    warnings.push(
      'Hardcoded UI text (healed mechanically after the build — no action needed):' +
