#!/usr/bin/env node
// Mechanical healer for the two record-shape TS2339 classes (run BEFORE
// `npm run build`: `node scripts/heal-tsc.mjs && npm run build`).
//
...
// (src/types/enriched.ts). Live builds keep slipping on exactly two spots,
// both provable from the generated type files — so they are pipeline input,
// not repair tasks:
//
//   A. `auftrag.auftragsnummer` where the field lives on
//      `auftrag.fields.auftragsnummer` — the missing `.fields.` prefix.
//      Fix: insert `fields.` before the property name (works for `?.` too).
//   B. `record: RechnungsPdfErstellen` in a type-literal member while the
//      body reads the Enriched-only field (`p.rechnungName`) — the raw type
//      where the data flowing in is enriched. Fix: retype the `record:`
//      member(s) to the Enriched* type and ensure its import.
//
// Both fixes only apply when the property PROVABLY exists at the target
// (parsed from the generated src/types/app.ts + src/types/enriched.ts) —
// no guessing, and everything else in the tsc output is passed through
...
function parseErrors(output) {
  ...
  for (const line of output.split('\n')) {
    const m = line.trim().match(ERROR_RE);
    if (!m) continue;
...
function healFile(filePath, errors, shapes) {
...
for (const [file, errs] of byFile) {
  ...
  try {
    fixed.push(...healFile(file, errs, shapes));
  } catch (e) {
...
if (!fixed.length) {
  ...
  console.log(JSON.stringify({ fixed: [], remaining: errors.length }));
  process.stdout.write(output);
  process.exit(1);
}

const second = runTsc();
console.log(JSON.stringify({ fixed, remaining: second.ok ? 0 : undefined }));
