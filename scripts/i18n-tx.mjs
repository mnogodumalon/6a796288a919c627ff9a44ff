...
// (src/i18n/pages.ts, generated server-side). This script's job is therefore
// pure CODE MOTION, never translation:
//
//   wrap <files...>     — hoist unsafe module-scope label consts into their
//                         component, then wrap every unmarked human literal
//                         in tx() at the proven positions (JSX text,
//                         localized attributes/props, ??/||/+= fallbacks,
//                         ternary branches, toasts/new Error, returns and
//                         assignments, template literals via the tx tag).
...
function scan(filePath, sourceOverride) {
  ...
  const visit = (node) => {
    ...
    if (ts.isJsxText(node)) {
      ...
      const humanStr = (b) => ts.isStringLiteral(b) && looksHuman(b.text) && !DATE_PATTERN.test(b.text);
      ...
      const bothCopy = branches.every((b) => ts.isStringLiteral(b) && b.text.trim() !== '') &&
      ...
      for (const b of branches) if (!ts.isStringLiteral(b)) visit(b);
      return;
    } else if (
  ...
  return { source, edits: kept, txKeys, residues, moduleScope, hoisted, intentSpans };
}
...
// Wrap needs the tx import; add it once, after the last import.
function ensureTxImport(src, filePath) {
  if (/import\s*\{[^}]*\btx\b[^}]*\}\s*from\s*'@\/i18n'/.test(src)) return src;
  const m = src.match(/import\s*\{([^}]*)\}\s*from\s*'@\/i18n';/);
  if (m) {
    return src.replace(m[0], `import { ${m[1].replace(/^[\s]*|[,\s]*$/g, '')}, tx } from '@/i18n';`);
  }
  ...
  return src.slice(0, importEnd) + "\nimport { tx } from '@/i18n';" + src.slice(importEnd);
}
...
if (!mode || files.length === 0) {
  console.error('usage: i18n-tx.mjs wrap|extract|intents|verify <files...>');
  process.exit(1);
}
