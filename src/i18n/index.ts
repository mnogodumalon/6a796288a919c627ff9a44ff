...
// The SOURCE TEXT is the key. The build language resolves to itself; every
// other language reads src/i18n/pages.ts (server-generated catalog) or, for
// overlay locales, locales/{lang}.json. Missing entries fall back to the
...
function resolvePage(key: string): string {
  ...
  if (isCore(locale)) return PAGES[locale]?.[key] ?? key;
  return (
    overlayFor(locale)?.pages?.[key] ??
    PAGES[BUILD_LOCALE === 'en' ? 'de' : 'en']?.[key] ??
    key
  );
}
