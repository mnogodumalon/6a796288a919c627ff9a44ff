...
import { formEnhancements } from '@/config/form-enhancements/Handwerker';
...
export default function HandwerkerDetailPage() {
  ...
  const [record, setRecord] = useState<Handwerker | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getHandwerker();
  ...
  async function handleUpdate(fields: Handwerker['fields']) {
    if (!record) return;
    await LivingAppsService.updateHandwerkerEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteHandwerkerEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/handwerker');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/handwerker')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/handwerker')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.vorname ?? appLabel('handwerker')} />
      ...
      <RecordSection title={t('details')} cols={2}>
        ...
        <RecordField label={fieldLabel('handwerker', 'qualifikation')} value={record.fields.qualifikation} format="pill" />
