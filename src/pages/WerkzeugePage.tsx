...
export default function WerkzeugePage() {
  ...
  const [records, setRecords] = useState<Werkzeuge[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Werkzeuge | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Werkzeuge | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getWerkzeuge());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Werkzeuge['fields']) {
  ...
  async function handleUpdate(fields: Werkzeuge['fields']) {
    ...
    await LivingAppsService.updateWerkzeugeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWerkzeugeEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('werkzeuge') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('werkzeuge') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('werkzeugname')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'werkzeugname')}
                  {sortKey === 'werkzeugname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('inventarnummer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'inventarnummer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kategorie')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'kategorie')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('hersteller')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'hersteller')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('seriennummer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'seriennummer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anschaffungsdatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'anschaffungsdatum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('foto')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'foto')}
                  {sortKey === 'foto' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bemerkungen_werkzeug')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('werkzeuge', 'bemerkungen_werkzeug')}
                  {sortKey === 'bemerkungen_werkzeug' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/werkzeuge/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.werkzeugname ?? '—'}</TableCell>
                <TableCell>{record.fields.inventarnummer ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('werkzeuge', 'kategorie', record.fields.kategorie?.key) ?? record.fields.kategorie?.label ?? '—'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('werkzeuge', 'zustand', record.fields.zustand?.key) ?? record.fields.zustand?.label ?? '—'}</span></TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.bemerkungen_werkzeug ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('werkzeuge') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
