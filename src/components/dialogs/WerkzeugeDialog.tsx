/**
 * WerkzeugeDialog — pre-generated create/edit dialog for Werkzeuge.
 *
 ...
 * WerkzeugeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Werkzeuge['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<WerkzeugeDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Werkzeuge, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Werkzeuge';
...
/** Widened prefill type for WerkzeugeDialog.defaultValues — see file header. */
export type WerkzeugeDialogDefaults = Omit<Werkzeuge['fields'], 'kategorie' | 'zustand'> & {
    kategorie?: LookupValue | string;
    zustand?: LookupValue | string;
  };

interface WerkzeugeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Werkzeuge['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: WerkzeugeDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['werkzeuge']?.['kategorie'] ?? [],
  zustand: LOOKUP_OPTIONS['werkzeuge']?.['zustand'] ?? [],
};
...
export function WerkzeugeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: WerkzeugeDialogProps) {
  const [fields, setFields] = useState<Partial<Werkzeuge['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['werkzeugname', 'inventarnummer', 'zustand'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Werkzeuge['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'werkzeuge');
      await onSubmit(clean as Werkzeuge['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "werkzeugname": string | null, // Werkzeugname\n  "inventarnummer": string | null, // Inventarnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "messgeraet" | "handwerkzeug" | "elektrowerkzeug" | "pruefgeraet" | "sicherheitsausruestung" | "sonstiges") mapping: messgeraet=Messgerät, handwerkzeug=Handwerkzeug, elektrowerkzeug=Elektrowerkzeug, pruefgeraet=Prüfgerät, sicherheitsausruestung=Sicherheitsausrüstung, sonstiges=Sonstiges\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell\n  "seriennummer": string | null, // Seriennummer\n  "anschaffungsdatum": string | null, // YYYY-MM-DD\n  "standort": string | null, // Standort / Lagerort\n  "zustand": LookupValue | null, // Aktueller Zustand (select one key: "verfuegbar" | "ausgeliehen" | "in_reparatur" | "in_wartung" | "defekt" | "ausgemustert") mapping: verfuegbar=Verfügbar, ausgeliehen=Ausgeliehen, in_reparatur=In Reparatur, in_wartung=In Wartung, defekt=Defekt, ausgemustert=Ausgemustert\n  "bemerkungen_werkzeug": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Werkzeuge['fields']>;
      });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        <Label htmlFor="kategorie">{fieldLabel('werkzeuge', 'kategorie')}</Label>
        <Select
          value={lookupKey(fields.kategorie) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, kategorie: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="kategorie" className="max-sm:h-11"><SelectValue placeholder="Messgerät, Handwerkzeug, Elektrowerkzeug..." /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="messgeraet">{lookupLabel('werkzeuge', 'kategorie', 'messgeraet') ?? 'Messgerät'}</SelectItem>
            <SelectItem value="handwerkzeug">{lookupLabel('werkzeuge', 'kategorie', 'handwerkzeug') ?? 'Handwerkzeug'}</SelectItem>
            <SelectItem value="elektrowerkzeug">{lookupLabel('werkzeuge', 'kategorie', 'elektrowerkzeug') ?? 'Elektrowerkzeug'}</SelectItem>
            <SelectItem value="pruefgeraet">{lookupLabel('werkzeuge', 'kategorie', 'pruefgeraet') ?? 'Prüfgerät'}</SelectItem>
            <SelectItem value="sicherheitsausruestung">{lookupLabel('werkzeuge', 'kategorie', 'sicherheitsausruestung') ?? 'Sicherheitsausrüstung'}</SelectItem>
            <SelectItem value="sonstiges">{lookupLabel('werkzeuge', 'kategorie', 'sonstiges') ?? 'Sonstiges'}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    ),
    'hersteller': (
      <div key="hersteller" className="space-y-1.5">
        ...
        <Input
          id="hersteller"
          placeholder="z. B. Bosch, DeWalt, Makita"
    ...
    'standort': (
      <div key="standort" className="space-y-1.5">
        ...
        <Input
          id="standort"
          placeholder="z. B. Werkstatt-Schrank C, Lagerraum 2"
    ...
    'zustand': (
      <div key="zustand" className="space-y-1.5">
        <Label htmlFor="zustand">{fieldLabel('werkzeuge', 'zustand')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
          value={lookupKey(fields.zustand) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, zustand: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="zustand" className="max-sm:h-11"><SelectValue placeholder="Verfügbar, Ausgeliehen, In Reparatur..." /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="verfuegbar">{lookupLabel('werkzeuge', 'zustand', 'verfuegbar') ?? 'Verfügbar'}</SelectItem>
            <SelectItem value="ausgeliehen">{lookupLabel('werkzeuge', 'zustand', 'ausgeliehen') ?? 'Ausgeliehen'}</SelectItem>
            <SelectItem value="in_reparatur">{lookupLabel('werkzeuge', 'zustand', 'in_reparatur') ?? 'In Reparatur'}</SelectItem>
            <SelectItem value="in_wartung">{lookupLabel('werkzeuge', 'zustand', 'in_wartung') ?? 'In Wartung'}</SelectItem>
            <SelectItem value="defekt">{lookupLabel('werkzeuge', 'zustand', 'defekt') ?? 'Defekt'}</SelectItem>
            <SelectItem value="ausgemustert">{lookupLabel('werkzeuge', 'zustand', 'ausgemustert') ?? 'Ausgemustert'}</SelectItem>
          </SelectContent>
        </Select>
    ...
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">{fieldLabel('werkzeuge', 'foto')}</Label>
        {fields.foto ? (
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeugname": "Werkzeugname", "inventarnummer": "Inventarnummer", "kategorie": "Kategorie", "hersteller": "Hersteller", "modell": "Modell", "seriennummer": "Seriennummer", "anschaffungsdatum": "Anschaffungsdatum", "standort": "Standort / Lagerort", "zustand": "Aktueller Zustand", "foto": "Foto des Werkzeugs", "bemerkungen_werkzeug": "Bemerkungen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WERKZEUGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
