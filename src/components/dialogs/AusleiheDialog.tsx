...
import type { Ausleihe, Werkzeuge, Handwerker, LookupValue } from '@/types/app';
...
export type AusleiheDialogDefaults = Omit<Ausleihe['fields'], 'status_ausleihe'> & {
    status_ausleihe?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status_ausleihe: LOOKUP_OPTIONS['ausleihe']?.['status_ausleihe'] ?? [],
};
...
export function AusleiheDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugeList, handwerkerList, enablePhotoScan = true, enablePhotoLocation = true }: AusleiheDialogProps) {
  ...
  const REQUIRED_FIELDS = ['werkzeug', 'handwerker', 'ausleihdatum', 'status_ausleihe'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "werkzeug": string | null, // Display name from Werkzeuge (see <available-records>)\n  "handwerker": string | null, // Display name from Handwerker (see <available-records>)\n  "ausleihdatum": string | null, // YYYY-MM-DDTHH:MM\n  "geplantes_rueckgabedatum": string | null, // YYYY-MM-DDTHH:MM\n  "tatsaechliches_rueckgabedatum": string | null, // YYYY-MM-DDTHH:MM\n  "status_ausleihe": LookupValue | null, // Status (select one key: "ausgeliehen" | "zurueckgegeben") mapping: ausgeliehen=Ausgeliehen, zurueckgegeben=Zurückgegeben\n  "bemerkungen_ausleihe": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["werkzeug", "handwerker"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'werkzeug': (
      <div key="werkzeug" className="space-y-1.5">
        ...
        <Combobox
          id="werkzeug"
          placeholder="Werkzeug aus Bestand wählen"
    ...
    'handwerker': (
      <div key="handwerker" className="space-y-1.5">
        ...
        <Combobox
          id="handwerker"
          placeholder="Welcher Handwerker leiht aus?"
    ...
    'ausleihdatum': (
      <div key="ausleihdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="ausleihdatum"
          placeholder="Wann beginnt die Ausleihe?"
          mode="datetime"
    ...
    'tatsaechliches_rueckgabedatum': (
      <div key="tatsaechliches_rueckgabedatum" className="space-y-1.5">
        ...
        <DatePicker
          ...
          placeholder="Tatsächlich zurückgebracht?"
          mode="datetime"
    ...
    'status_ausleihe': (
      <div key="status_ausleihe" className="space-y-1.5">
        <Label htmlFor="status_ausleihe">{fieldLabel('ausleihe', 'status_ausleihe')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_ausleihe) === 'ausgeliehen'}
            onClick={() => setFields(f => ({ ...f, status_ausleihe: (lookupKey(f.status_ausleihe) === 'ausgeliehen' ? undefined : 'ausgeliehen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_ausleihe) === 'ausgeliehen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihe', 'status_ausleihe', 'ausgeliehen') ?? 'Ausgeliehen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_ausleihe) === 'zurueckgegeben'}
            onClick={() => setFields(f => ({ ...f, status_ausleihe: (lookupKey(f.status_ausleihe) === 'zurueckgegeben' ? undefined : 'zurueckgegeben') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_ausleihe) === 'zurueckgegeben'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihe', 'status_ausleihe', 'zurueckgegeben') ?? 'Zurückgegeben'}
          </button>
        </div>
        {showErrors && !fields.status_ausleihe && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'bemerkungen_ausleihe': (
      <div key="bemerkungen_ausleihe" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Zustand beim Verleihen, besondere Bedingungen…"
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeug": "Werkzeug", "handwerker": "Handwerker", "ausleihdatum": "Ausleihdatum und -uhrzeit", "geplantes_rueckgabedatum": "Geplantes Rückgabedatum und -uhrzeit", "tatsaechliches_rueckgabedatum": "Tatsächliches Rückgabedatum und -uhrzeit", "status_ausleihe": "Status", "bemerkungen_ausleihe": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"werkzeug": {"werkzeugname": "Werkzeugname", "inventarnummer": "Inventarnummer", "kategorie": "Kategorie", "hersteller": "Hersteller", "modell": "Modell", "seriennummer": "Seriennummer", "anschaffungsdatum": "Anschaffungsdatum", "standort": "Standort / Lagerort", "zustand": "Aktueller Zustand", "foto": "Foto des Werkzeugs", "bemerkungen_werkzeug": "Bemerkungen"}, "handwerker": {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "abteilung": "Abteilung", "qualifikation": "Qualifikation / Rolle", "status": "Status", "bemerkungen": "Bemerkungen"}};
