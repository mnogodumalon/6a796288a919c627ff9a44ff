...
async def _build_intent_pages(args: dict) -> dict:
    ...
    lines += ["", "Next: wire each page that built with scripts/wire-intent.mjs, then `node scripts/heal-tsc.mjs && npm run build` (heal-tsc mechanically fixes the missing-.fields.-prefix, raw-vs-Enriched and ??-null-in-payload classes; fix only what it prints below its JSON report)."]
