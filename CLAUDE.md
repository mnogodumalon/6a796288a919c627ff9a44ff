...
**UI TEXT (three languages):** The dashboard ships de/en/cs with a runtime language switcher — a hardcoded string stays frozen in one language while everything around it switches. Scaffold text is already localized: read it via `t()`, `appLabel()`, `fieldLabel()`, `lookupLabel()` from `@/i18n`, never re-type it. Every string YOU introduce goes through `makeT`: define ALL three languages once, render `{tt('key')}` (check-dashboard gate 21 enforces this; deliberate exceptions like brand names take `/* i18n-exempt */` on the line).
WRONG: `<h2>Auslastung</h2>`
RIGHT: `const tt = makeT({ de: { util: 'Auslastung' }, en: { util: 'Utilization' }, cs: { util: 'Vytížení' } });` … `<h2>{tt('util')}</h2>`
...
- All scaffold UI text ships in de/en/cs with a runtime switcher (`src/i18n`) — chrome via `t()`, entity/field/lookup labels via `appLabel()`/`fieldLabel()`/`lookupLabel()`
...
| `src/i18n/index.ts` | Runtime language layer (de/en/cs): `t()`, `makeT()`, `appLabel()`, `fieldLabel()`, `lookupLabel()`, formats; follows the platform header's language switch via `<html lang>` — DO NOT edit |
