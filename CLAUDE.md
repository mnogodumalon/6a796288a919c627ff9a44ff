...
**UI TEXT (multilingual):** The dashboard ships de/en with a runtime language switcher (more languages can be added later without a rebuild) — a hardcoded string stays frozen in one language while everything around it switches. Scaffold text is already localized: read it via `t()`, `appLabel()`, `fieldLabel()`, `lookupLabel()` from `@/i18n`, never re-type it. Every string YOU introduce goes through `makeT`: define de AND en once, render `{tt('key')}` (check-dashboard gate 21 enforces this; deliberate exceptions like brand names take `/* i18n-exempt */` on the line).
WRONG: `<h2>Auslastung</h2>`
RIGHT: `const tt = makeT({ de: { util: 'Auslastung' }, en: { util: 'Utilization' } });` … `<h2>{tt('util')}</h2>`
