...
| `scripts/heal-tsc.mjs` | Run before every build (`node scripts/heal-tsc.mjs && npm run build`) — mechanically fixes the missing-`.fields.`-prefix, raw-vs-Enriched and `?? null`-in-payload classes |
