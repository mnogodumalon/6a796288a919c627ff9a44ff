...
Erst NACH Barriere UND Parser-Lauf: `node scripts/heal-tsc.mjs && npm run build`. heal-tsc runs tsc itself and mechanically repairs the two record-shape error classes (a missing `.fields.` prefix; a `record:` member typed with the raw type while an Enriched* field is read) — whatever it reports as remaining you fix yourself, then retry until the build succeeds.
...
| `scripts/heal-tsc.mjs` | Run before every build (`node scripts/heal-tsc.mjs && npm run build`) — mechanically fixes the missing-`.fields.`-prefix and raw-vs-Enriched TS2339 classes |
