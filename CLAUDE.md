...
**UI TEXT (multilingual):** The dashboard ships de/en with a runtime language switcher (more languages can be added later without a rebuild) — a hardcoded string stays frozen in one language while everything around it switches. Scaffold text is already localized: read it via `t()`, `appLabel()`, `fieldLabel()`, `lookupLabel()` from `@/i18n`, never re-type it. Standard words (Überfällig, Bearbeiten, Abschließen, toast verbs, …) are pre-translated in `tc` from `@/i18n/common` — use its keys (listed in its header in your context), never re-type those words either. Every string YOU introduce beyond that goes through `makeT`: define de AND en once, render `{tt('key')}` (deliberate exceptions like brand names take `/* i18n-exempt */` on the line). This includes template-literal toasts: `` undoToast(`${name} — ${tc('zurueckgegeben')}`, …) ``, never `` `${name} zurückgegeben` ``.
WRONG: `<h2>Auslastung</h2>` … `label: 'Bearbeiten'`
RIGHT: `const tt = makeT({ de: { util: 'Auslastung' }, en: { util: 'Utilization' } });` … `<h2>{tt('util')}</h2>` … `label: tc('bearbeiten')`
Derive anything that reads `LOOKUP_OPTIONS` labels INSIDE the component body, never at module scope — the labels are locale-aware getters and a module-scope `.map(o => ({ label: o.label }))` freezes one language at import (gate 22).
...
- **DashboardOverview.tsx** — You MUST call `Read("src/pages/DashboardOverview.tsx")` FIRST. Then call `Write` ONCE with the complete new content. Do NOT read it back after writing. If a gate then flags a line, repair it with `Edit` — a second full `Write` of this page is the single most expensive mistake in the run. Do NOT use Bash cat/echo — use ONLY Read and Write tools. The skeleton already has `useDashboardData()`, enrichment, loading/error — keep that pattern, replace the empty content div. **Keep the `DashboardSkeleton`/`DashboardError` import from `@/components/DashboardStates` and the two early-returns** — never write your own loading/error components (check-dashboard gate 20 fails the build). **Keep the enriched type imports** (`import type { EnrichedX } from '@/types/enriched'`) and enrichment calls (`const enrichedX = enrichX(x, { ... })`) from the skeleton — they are pre-generated for the specific entities that have applookup dependencies. **Define your page texts via `makeT` from `@/i18n` and standard words via `tc` from `@/i18n/common`** (de AND en, see UI TEXT rule) — gate 21 lists every hardcoded string it finds.
...
| `src/i18n/index.ts` | Runtime language layer (de/en + overlay locales): `t()`, `makeT()`, `appLabel()`, `fieldLabel()`, `lookupLabel()`, formats; follows the platform header's language switch via `<html lang>` — DO NOT edit |
| `src/i18n/common.ts` | `tc()` — pre-translated standard vocabulary (Überfällig, Bearbeiten, toast verbs, …); use its keys instead of re-typing the words — DO NOT edit |
