...
UI TEXT (three languages): public pages follow the visitor's browser language
(de/en/cs). Every UI string you write goes through `makeT` from `@/i18n` —
define all three languages once, render `{tt('key')}`. Visitor-facing Czech
uses formal address (vykání). `@/i18n` is anonymous-safe and allowlisted in
check-public.
