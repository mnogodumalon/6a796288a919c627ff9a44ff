...
(de/en; more languages attach later as overlays). Every UI string you write
goes through `makeT` from `@/i18n` — define de AND en once, render
`{tt('key')}`. `@/i18n` is anonymous-safe and allowlisted in check-public.
