...
// tt = your page's makeT table; tc = pre-translated standard words from '@/i18n/common'
<DashboardGrid
  hero={überfällig.length > 0 && (
    <HeroBanner icon={<IconAlertTriangle size={18} />} action={{ label: tt('fertig_melden'), onClick: () => advance(überfällig[0]) }}>
      <b>{namen(überfällig.map(r => r.kundeName ?? ''))}</b> — {tc('ueberfaellig')}: {formatDate(überfällig[0].fields.faellig)}.
    </HeroBanner>
  )}
  kpis={<StatCardRow>…2–4 StatCards…</StatCardRow>}
  aside={<WorkList title={tt('heute_faellig')} items={…} onItemClick={id => overlay.replace({ id })} empty={{ text: …, action: … }} />}
...
<RecordKeyFacts items={[
  { label: tt('brutto'), value: brutto != null ? formatCurrency(brutto) : '—' },
]} />
```
...
```tsx
<WorkList
  title={tt('faellig_heute')}
  items={faellige.map(r => ({
    id: r.record_id,
    title: r.kundeName ?? r.fields.kennzeichen,                                 // the person/thing
    secondLine: <><span className="font-medium text-destructive">{tc('ueberfaellig')}</span><span className="text-muted-foreground"> · {formatDate(r.fields.faellig)}</span></>,
    action: { label: `✓ ${tc('weiter')}`, onClick: () => advance(r) },          // the SAME shared helper
  }))}
  onItemClick={id => overlay.replace({ id })}
  empty={{ text: tt('alles_im_plan'), action: { label: tt('neuer_auftrag'), onClick: openCreate } }}
/>
...
- **The hub overlay shows ALL satellites via `<SatelliteSection>`** (pre-generated, `@/components/SatelliteSection`) — one per entry in `HUB_TOPOLOGY[hubKey]`. **The cheap way is to compose the generated `<{Hub}Details>` block**: it already renders exactly those sections (one per incoming applookup edge) plus the record's field details, and `check-hub.mjs` counts them inside that file. So the hub branch of your overlay is usually one line — `<WerkzeugeDetails record={rec} werkzeugausgabeList={…} onOpenWerkzeugausgabe={…} onAddWerkzeugausgabe={…} />` — and you are done. Hand-render the section yourself only for a relation the block does not carry, and never for one it already has (it would render twice). The component bakes in the three mechanics that are easy to get wrong by hand, so they're guaranteed either way: it always renders the "+" (the `onAdd` prop is required), a row click fires `onOpen` (→ `overlay.push` the DETAIL, never the edit form), and the header-count + relation-list + dashed-add layout is consistent. The hand-wired shape, for the exception case:
  ```tsx
  <SatelliteSection
    title={appLabel('maengelerfassung')}
    items={maengelVon(b.record_id)}
    getKey={m => m.record_id}
    map={m => ({ label: `${m.fields.prio?.label ?? ''} · ${m.fields.status?.label ?? ''}`, name: m.fields.typ, meta: m.fields.beschreibung, icon: IconAlertTriangle })}
...
- **Cover EVERY satellite in `HUB_TOPOLOGY`, not just the obvious ones.** Loop history: wiring only Mängel → the user immediately notices "I can only add defects"; wiring a row click to the edit dialog → "the dialog that opens doesn't fit". `SatelliteSection` + `HUB_TOPOLOGY` + `check-hub.mjs` make both impossible — and composing `<{Hub}Details>` covers all of them at once, which is why that is the default. What you DO still owe per satellite is the wiring the block asks for: its `onOpen{Sat}` must `overlay.push` that satellite's own overlay branch, and its `onAdd{Sat}` must open that entity's create dialog with the hub pre-set — so import the create dialog for ALL satellite entities (Mängel AND VE AND Fotos AND Kommunikation AND Checklisten AND Leitungen). Passing one satellite's handler to another's prop type-checks and then opens the wrong sheet.
...
<ConfirmDialog
  open={!!deleteTarget}
  title={tt('loeschen_titel')}
  description={tt('loeschen_frage')}
