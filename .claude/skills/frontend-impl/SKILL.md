...
```bash
node scripts/heal-tsc.mjs && npm run build
```

heal-tsc runs tsc and mechanically repairs the two record-shape error classes
(missing `.fields.` prefix; `record:` typed raw while an Enriched* field is
read) — fix only what it reports as remaining.
